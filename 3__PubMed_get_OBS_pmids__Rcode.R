...
obs_r_search <- entrez_search(db="pubmed", term="(study) AND (Observational Study[Publication Type])", retmax = 1,
								api_key = "af82cca6d1bd01f71dc4319bf810722a2708", use_history = FALSE)
# URL with the data (PMIDs)
# (study) AND (Observational Study[Publication Type]) -> term=%28study%29+AND+%28Observational+Study%5BPublication+Type%5D%29
# (study) AND (Meta-Analysis[Publication Type])		  -> term=%28study%29+AND+%28Meta-Analysis%5BPublication+Type%5D%29
obs_base_url <- "https://pubmed.ncbi.nlm.nih.gov/?term=%28study%29+AND+%28Observational+Study%5BPublication+Type%5D%29&api_key=af82cca6d1bd01f71dc4319bf810722a2708"
obs_first_url <- str_glue("{obs_base_url}&sort=pubdate&sort_order=asc&size=10")
obs_last_url <- str_glue("{obs_base_url}&sort=pubdate&sort_order=desc&size=10")
# Number of studies
total_count <- obs_r_search$count |> as.integer()
obs_ids <- rep(NA, total_count)
# Get the year of the first publication
search_rslt <- read_html(obs_first_url)
...
month = 0
current_position = 0
# Not a good practice at all, such a large query should use FTP, -> just a temporary solution.
repeat {
	# Prepare
	monthly_ids <- "empty"
	# Last month search
	if (month == 12) {
		monthly_obs_r_search <- tryCatch({ entrez_search(db="pubmed", term=str_glue("(study) AND (('{year}/{month}/1'[Date - Publication] : '{year}/{month}/31'[Date - Publication])) AND (Observational Study[Publication Type])"),
	...
	# General search
	if (month != 12) {
		monthly_obs_r_search <- tryCatch({ entrez_search(db="pubmed", term=str_glue("(study) AND (('{year}/{month}/1'[Date - Publication] : '{year}/{month+1}/1'[Date - Publication])) AND (Observational Study[Publication Type])"),
														retmax = 9999, api_key = "af82cca6d1bd01f71dc4319bf810722a2708", use_history = FALSE) },
										warning = function(w) { "empty" },
										error = function(e) { "empty" })
	}
	# Collect the IDs
	if (is.list(monthly_obs_r_search)) {
		monthly_ids <- monthly_obs_r_search$ids
		if (length(monthly_ids) > 0) {
			for (i in seq(1:length(monthly_ids))) {
				obs_ids[current_position + 1] <- monthly_ids[i]
...
obs_ids_t <- tibble(pmid = obs_ids) |> filter(!is.na(obs_ids)) |> distinct()
write_tsv(obs_ids_t, str_glue("{out_path}obs_ids__12-08-2025.tsv"))